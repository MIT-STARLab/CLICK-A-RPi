...
def send_heartbeat(current_time):
    ...
    if workload:
        ...
        if(CMD_ID == APID_TIME_AT_TONE):
            ...
            while(abs(end_time - start_time) < TRANSMIT_TIME):
                ...
                end_time = time.time()
                if((end_time - start_time) >= HK_CH_HEARTBEAT_PD*counter_heartbeat): send_heartbeat(end_time)
