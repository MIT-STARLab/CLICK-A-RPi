...
def pwr_edfa_on(fo):
    ...
    time.sleep(0.1)
    fpga.write_reg(mmap.EDFA_IN_STR ,'ldc ba 600\r')
    time.sleep(0.1)
...
def run_all(origin):
    ...
    with file_manager.ManagedFileOpen('/root/log/%s.gz' % t_str,'w') as (f, tags):
        ...
        tags['origin'] = origin
        f.write('EDFA ON Script, %s\n' % t_str)
        print('   CLICK-A EDFA ON Script')
        try:
            def file_as_bytes(file):
                with file:
                    return file.read()

            hash_v =  hashlib.md5(file_as_bytes(open('/root/test/edfa_on.py', 'rb'))).hexdigest()
            f.write('MD5: %s\n' % str(hash_v))
